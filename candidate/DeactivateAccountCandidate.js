...
import { deactivatecandidateAPI } from "../../../api/authCandidate";

const DeactivateAccountCandidate = () => {
  ...
  const deactivateAccount = async () => {
    try {
      const { data, status, message } = await deactivatecandidateAPI(
        candidateId
      );
      if (status) {
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white  text-left align-middle shadow-xl transition-all">
                  <div className="flex gap-2 justify-between px-6 pt-6">
                    ...
                    >
                      Your account is get deactivate.
                    </Dialog.Title>
                    <div>
                      <ImCross
...
export default DeactivateAccountCandidate;
