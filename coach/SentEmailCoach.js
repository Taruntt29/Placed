...
import { getAllSentEmails, moveToTrashById } from "../../../api/coachFunctions";
import { toast } from "react-toastify";
import { useEffect } from "react";
import { useSelector } from "react-redux";
 
const SentEmailCoach = () => {
  ...
  const fromMail = userDetails.email;
  const [mailData, setMailData] = useState([]);
  // const [toMail, setToMail] = useState();
  const getAllSentMails = async () => {
    const { data, message, status } = await getAllSentEmails(fromMail);
    if (status) {
      setMailData(data);
        ...
        <div className="container mx-auto pt-3  shadow-email mb-12  mt-5 rounded">
          <div className="flex space-x-4  px-5 pb-3">
            ...
            <div className="">
              <Menu as="div" className="relative inline-block text-left">
                ...
                >
                  <Menu.Items className="absolute  py-3 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="px-3  ">
          ...
          {mailData &&
            mailData.map((item, index) => (
              <div
                className="lg:grid grid-cols-12  border-gray-200 border-t-2 p-4 gap-2"
                key={index}
              >
