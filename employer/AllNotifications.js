...
const AllNotification = () => {
  ...
  const getnotification = async () => {
    try {
      const { data, status, message } = await getAllNotificationAPI(entryform);
      if (status) {
  ...
  const deleteNotification = async () => {
    try {
      const { data, status, message } = await deleteNotificationAPI(
        notificationId
      );
      if (status) {
        getnotification();
              ...
              <div className="bg-inputcolor m-6  rounded-md">
                <div className="md:grid grid-cols-12   lg:gap-16 p-4 ">
                  ...
                  <div className="col-span-1">
                    <div className="p-2 bg-secondary my-3 flex justify-center items-center rounded">
                      <RiDeleteBin6Line
