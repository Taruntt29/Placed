...
import { getInboxAssistantEmailAPI } from "../../../api/authService";

const SupportInboxEmployer = () => {
  ...
  const getAllInboxMails = async () => {
    const { data, message, status } = await getInboxAssistantEmailAPI(toMail);
    if (status) {
      setMailData(data);
                ...
                >
                  <Menu.Items className="absolute pt-2  mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="px-3  ">
                      ...
                      <Link to="/employer/support-sent-mail">
                        <Menu.Item>
                          {({ active }) => (
                            <div
                              className={`${
...
export default SupportInboxEmployer;
