...
import { deleteEmailAPI, getEmailSentEmployer } from "../../../api/authService";
...
const SentEmail = () => {
  ...
  // console.log("jsgdjs", id);
  const [inboxData, setInboxData] = useState([]);

  // API
  const InboxDetails = async () => {
    const { data, message, status } = await getEmailSentEmployer(from);
