...
import { getAssistantMailDetailById } from "../../../api/authService";
const SupportReplyEmployer = () => {
  ...
  const getEmailDetailById = async () => {
    const { data, status, message } = await getAssistantMailDetailById(
      state.mailId
    );
    if (status) {
      setMailData(data);
...
export default SupportReplyEmployer;
