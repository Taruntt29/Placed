...
import { postreplyAssistantMail } from "../../../api/authService";
const SupportComposeEmployer = () => {
  ...
  const sendMail = async () => {
    ...
    if (userDetails.accountstatus === "Active") {
      try {
        const response = await postreplyAssistantMail({
          body: stateData.body,
        ...
        if (response.status) {
          ...
          navigate("/employer/support-inbox-mail");
        } else {
          toast(response.message);
        }
      } catch (error) {
...
export default SupportComposeEmployer;
