...
import { employerSignupAPI } from "../../../api/authService";
  ...
  const handleSignup = async () => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      ...
      if (createPassword != confirmPassword) {
        ...
        try {
          const response = await employerSignupAPI({
          ...
          if (response.status) {
            navigate("/employer-signup-otp", { state: { email: email } });
              ...
              <div className="flex justify-start w-full gap-3 flex-col">
                ...
                <div className="flex justify-start flex-col gap-1 w-full mobileinput">
                  ...
                  <PhoneInput
                    ...
                    value={value}
                    onChange={setValue}
                    className="w-full bg-inputcolor text-sm placeholder:text-[#000000] placeholder:text-opacity-30 placeholder:text-sm px-3 py-2 rounded-[7px]"
                  />
                  {/* {value.length < 10 ? (
